	...
	*/
    ...
    #region Индексы
		...
		/*
			...
			Вместо перелистываия всех страниц в поисках главы, открываем оглавление и потом бинарным поиском ищем нужную страницу
			Если сделать много индексов по разным столбцам, то при изменении таблицы будут изменяться все индексы - это замедлит БД
			...
			Виды некластерных индексов:
				...
				B-дерево (B-tree):
					...
					- эффективные операциии поиска, вставки и удаления, диапазонных запросов
    ...
    #region ACID
		...
		/*
			...
			Св-ва обеспечивают надежность и целостность транзакций:
    ...
    #region Транзакции
		...
		#region Фиксация или откат транзакции в зависимости от условий
    ...
    #region План выполнения запроса (ПВЗ)

        /*
			ПВЗ (Query Execution Plan) определяет:
			...
			Включает шаги:
			- Парсинг запроса
			  СУБД проверяет синтаксис на корректность
			- Оптимизация запроса
			  СУБД оптимизирует запрос, выбирая наилучший ПВЗ
			  Учитываются индексы, статистика таблиц, доступные ресурсы
			- Генерация ПВЗ
			  СУБД создает ПВЗ, который описывает порядок операций для получения данных
			  ПВЗ может включать выборку данных из таблиц, использование индексов, сортировку, объединение данных
			- Выполнение ПВЗ
			  СУБД выполняет ПВЗ и возвращает результат

			Как работать с ПВЗ:
			- Изучите ПВЗ, чтобы понять, как СУБД его выполняет, какие индексы используются, какие операции выполняются
			- Просмотрите ПВЗ, чтобы определить, какие индексы используются или не используются при выполнении запроса
			- В некоторых случаях СУБД может выбрать неоптимальный ПВЗ
			  Можем использовать подсказки в SQL запросе, чтобы указать СУБД на определенный план выполнения
			- Регулярно анализировать ПВЗ для отслеживания производительности
		*/
    ...
    #region Преимущества и недостатки хранимых процедур (ХП) по сравнению с запросами

        /*
			Преимущества ХП:
				- компилируются и оптимизируются на сервере БД,
				  что быстрее по сравнению с динамическими SQL-запросами,
				  которые компилируются каждый раз при выполнении
				- могут выполнять сложные операции на стороне БД,
				  что уменьшает объем данных, передаваемых между приложением и БД
				- можно предоставить пользователям доступ к выполнению процедур
				  без предоставления прямого доступа к таблицам, что более безопасно

			Недостатки ХП:
				- ХП часто зависят от конкретной реализации СУБД,
				  что может затруднить переносимость приложения между различными системами
			...
			Преимущества запросов среднего слоя:
			...
			Недостатки запросов со среднего слоя:
				- динамические запросы могут быть менее эффективными по сравнению с хранимыми процедурами,
				  если часто выполняются с одинаковыми параметрами
				- если приложение выполняет множество отдельных запросов для получения данных,
				  растет объем передаваемых данных
				- необходимость управления доступом к данным на уровне приложения может привести к уязвимостям,
				  если не реализованы механизмы безопасности
		*/
    ...
    #region Оконная функция
		...
		/*
			Внутри ф-ий нельзя выполнять транзакции, только в хранимых процедурах
			...
			Пример 1:
				...
				Хотим получить список сотрудников с их уникальными номерами в пределах каждого отдела, отсортированным по зарплате:
