public class A
{
	...
	*/
	...
	/*
		...
		- Read uncommitted	- даже если данные еще не закоммичены в рамках одной транзакции,
							  мы все-равно можем их прочитать из другой транзакции
		...
		- Snapshot			- если две транзакции совершаются прааллельно, то каждой из них выдают свой слепок БД
							  в определенный момент. После выполнения всех операций со слепком БД
							  (удаление/запись/изменение/чтение) все изменения вливаются в основную версию БД
							  Транзакция будет завершена успешно, если в основной версии БД к моменту окончания
							  транзакции ни в одной из ячеек базы, измененных в ходе транзакции, не было изменений
							  за время ее выполнения. Так, если две транзакции выполняли операции над разными частями
							  БД, то конфликтов не возникнет и слияние произойдет безболезненно
							  Если изменялись одни и те же данные - можем получить аномалию косой записи
	*/
	#endregion
