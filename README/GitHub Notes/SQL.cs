public class A
{
	#region Разное
		...
		create index index1 on dbo.Table1(number);			// добавление индекса для поля существующей таблицы
	...
	/*
		...
		WHERE
			фильтрует строки на основе условий, указанных в WHERE

		HAVING
			фильтрует сгруппированыме по определенным столбцам строки на основе агрегатных функций COUNT, SUM, AVG

		Найдем клиентов с суммой заказов > 1000р.
		Группируем по "customer_id" и вычисляем сумму заказов функцией SUM
		Применяем оператор HAVING, чтобы отфильтровать группы строк, у которых сумма заказов > 1000р.
	...
	*/
	...
	#region ACID (Atomicity, Consistency, Isolation, Durability)
	/*
	...
	*/
	...
	// join
	select Name, Title from Authors a		// Возвращает совпадения из обеих таблиц
		join Books b
		on a.Id = b.AuthorId
	...
	// full join
	select Name, Title from Authors a		// Возвращает все записи из обеих таблиц
		full join Books b
		on a.Id = b.AuthorId
