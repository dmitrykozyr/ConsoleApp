...
docker build --tag [имя iamge маленькими буквами] .	// создать imagedocker build --tag [имя маленькими буквами] .	// создать image
docker images                                       // список images
docker run -p 8080:8081 [имя или id image] --rm -it // запускаем контейнер на основе образа (image) и указываем два порта:
                                                    // - первый - локальный
