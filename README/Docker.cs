Традиционно веб-приложения развертывались на сервере, а в последнее время – в виртуальной машине
Виртуальные машины тяжелы с точки зрения размера файла и с точки зрения ресурсов
Контейнеры намного легковеснее и не имеют больших накладных расходов
Состоят из нескольких уровней и не требуют загрузки новой операционной системы при запуске


1. Containers (Контейнеры) — изолированные экземпляры приложений, которые работают на основе образов
   ...
   Могут быть временными и легко создаваться и удаляться

2. Images (Образы) — статические файлы, которые содержат необходимые компоненты для запуска контейнера,
   включая код приложения, библиотеки, зависимости и настройки
...
3. Volumes (Тома) — механизмы хранения данных, которые используются контейнерами
...
4. Builds (Сборки):
   ...
   Dockerfile — текстовый файл, который содержит инструкции по созданию образа,
   такие как установка ПО, копирование файлов и настройка окружения
