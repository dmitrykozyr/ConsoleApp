...
{
    /*
        Документно-ориентированная NoSQL БД, которая хранит данные в формате BSON (Binary JSON)

        Обеспечивает:
        - высокую производительность
        - масштабируемость
        - гибкость при работе с большими объемами данных

        Данные хранятся в виде документов, которые представляют собой структуры, подобные JSON - позволяет хранить сложные и вложенные структуры данных
        Не требует строгой схемы и документы в одной коллекции могут иметь разные поля и структуры, что упрощает работу с изменяющимися требованиями
        Поддерживает горизонтальное масштабирование, что позволяет распределять данные по нескольким серверам и увеличивать производительность
        Оптимизирована для работы с большими объемами данных и обеспечивает высокую скорость чтения/записи благодаря индексам и кэшированию
        Язык запросов позволяет выполнять сложные операции, такие как фильтрация, агрегация и сортировка данных
        Позволяет создавать реплики (копии) данных на нескольких серверах
        Выполняет сложные вычисления и преобразования данных на уровне БД
     */
    ...
    #region Получение данных
        ...
        // Вывести документы, где 'name=Tom'
        db.users.find({ name: "Tom"})
 
        // Вывести документы, где 'name=Tom' и 'age=32'
        db.users.find({ name: "Tom", age: 32})
 
        // Вывести документы, где св-во 'languages' пустое
        db.users.find({ languages: null})
 
        // Вывести документы, где в массиве 'languages' есть 'english'
        db.users.find({ languages: "english"})
 
        // Вывести документы, где в массиве 'languages' есть 'english' и 'german'
        db.users.find({ languages: ["english", "german"]})
 
        // Вывести документы, где 'english' в массиве 'languages' находится на ПЕРВОМ месте
        // 'languages.0' предоставляет сложное свойство и поэтому берется в кавычки
        db.users.find({ "languages.0": "english"})
 
        // Вывести документы, где 'english' в массиве 'languages' находится на ВТОРОМ месте
        // 'languages.0' предоставляет сложное свойство и поэтому берется в кавычки
        db.users.find({ "languages.1": "english"})
 
        // Вывести значения полей 'age' у всех документов, где 'name=Tom'
    ...
    #region Вставка данных
        ...
        db.users.insertOne(
        {
        "_id": ObjectId("62e27b1b06adfcddf4619fc6"),
            "name": "Tom",
            "age": 28,
            "languages": ["english", "spanish"]
        })
    ...
    #region Индексы
        ...
        // Тогда все добавляемые документы должны иметь уникальные значения для обоих полей
    ...
    #region Поиск по массивам

        // all определяет набор значений, которые должны быть в массиве
    ...
    #region Обновление данных
        ...
        // replaceOne заменяет один документ другим
        // - filter - запрос на выборку документа, который надо обновить
        // - update - новый документ, который заместит старый
        // - options - дополнительные параметры, основным из которых является параметр upsert
        
        // upsert == true   документ обновится, если он найден, иначе будет создан новый
        // upsert == false  новый документ не будет создан, если не будет найден
        db.collection.replaceOne(filter, update, options)

        // Находим документ с 'name=Bob' и заменяем его на' {name: "Bob", age: 25}'
        db.users.replaceOne({ name: "Bob"}, { name: "Bob", age: 25})
 
        // Обновим не весь документ, а одно или несколько его свойств:
        // - updateOne обновляет только один документ
        // - updateMany обновляет множество документов
        // Для обновления отдельных полей применяется оператор set
    ...
    #region Обновление массивов
 
         // push добавить значение к существующему,
         ...
         // each добавляет несколько значений
         ...
         // В данном случае элементы ["german", "spanish", "italian"] будут вставляться в массив 'languages' с 1-го индекса,
         // а после вставки в массиве останутся только 5 первых элементов
         ...
         // но еще добавляет данные, если их еще нет в массиве,
         ...
         // Указывая для ключа 'languages' значение 1, удаляем первый элемент с конца массива
         ...
         // Например, через push можем добавить значение в массив несколько раз
         // И теперь с помощью pull удалим его
         db.users.updateOne({ name: "Tom"}, {$pull: { languages: "english"} })

         // pullAll удаляет не одно значение, а несколько
    ...
    #region Установка ссылок в БД
        ...
        // РУЧНАЯ УСТАНОВКА ССЫЛОК
            ...
            // Добавим в коллекцию 'companies' документ, представляющий компанию
             ...
             // Важно, что в качестве значения для этого поля устанавливаем не объект 'company', а значение ключа '_id'
            ...
            user = db.users.findOne()
             ...
             // Найдем ссылку на его компанию в коллекции 'companies'
         ...
         // АВТОМАТИЧЕСКОЕ СВЯЗЫВАНИЕ
             ...
             // При добавлении нового документа генерирует '_id', который можем получить с помощью свойства 'insertedId' результата функции
             // Создадим новый документ для коллекции 'users', у которого ключ 'company' свяжем с только что добавленным документом 'google'
            ...
            _id: sam.company.$id
             ...
             // Надо конкретизировать параметр 'sam.company.$id', т.к. 'sam.company' теперь представляет объект 
             { "$ref" : "companies", "$id" : google.insertedId}
    ...
    #region Управление коллекцией
        ...
        // ЯВНОЕ СОЗДАНИЕ КОЛЛЕКЦИИ
            ...
            // Можем создать ее явно, применив метод db.createCollection(name, options),
        ...
        // ОГРАНИЧЕННЫЕ КОЛЛЕКЦИИ
            ...
            // При обновлении документов в таких коллекциях документы не должны расти в размерах, иначе обновление не удастся
            // Нельзя удалять документы из подобных коллекций, можно только удалить всю коллекцию
 
        // ПОДКОЛЛЕКЦИИ

            // Упрощают организацию данных в коллекциях
