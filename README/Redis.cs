...
информация изначально находится в структурах данных, которые нужны программисту
 ...
 1.Клиент обращается к приложению, оно получает необходимые для выполнения запроса данные
 ...
 Redis гарантирует высокий уровень сохранности данных,
 что позволяет использовать эту СУБД в роли основной БД
 Так-же Redis поддерживает плагины, улучшающие работу
 Так-же эта СУБД размывает границы между кешем и хранилищем данных
 ...
 Redis позволяет настроить, как данные сохраняются на диск

 В начале эта система использовала снепшоты, когда асинхронные копии данных из памяти
 отправляли на диск для долговременного хранения
 Недостаток в возможной потере данных, изменённых или добавленных в хранилище на временных интервалах между снепшотами
