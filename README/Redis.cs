...
Redis (Remote Dictionary Service) — OpenSource сервер БД типа ключ-значение
Это сервер структур данных
Вместо того чтобы работать со строками БД, перебирать, сортировать, упорядочивать,
...
Данные из нее загружаются в кеш, после чего ими сможет воспользоваться приложение
 ...
 Самый простой вариант развёртывания
 ...
 копии соответствующих команд отправляются в выходной буфер для подчинённых узлов, что обеспечивает репликацию
