...
class Tasks
{
    ...
    #region Разница между Task.Delay(5) и Thread.Sleep(5)

    /*

    1. Асинхронность vs. Синхронность

        Thread.Sleep(5) - синхронный метод, который блокирует текущий поток на указанный промежуток времени
        ...
        Task.Delay(5) - асинхронный метод, который не блокирует поток, а возвращает Task, который завершится через указанное время
    ...
    2. Использование ресурсов
        ...
        Thread.Sleep(5) блокирует поток и может привести к снижению производительности, особенно если используется в UI-приложениях,
        так как это может вызвать зависание интерфейса пользователя

        Task.Delay(5) не блокирует поток, что позволяет более эффективно использовать ресурсы и поддерживать отзывчивость приложения

    3. Возврат результата

        Thread.Sleep(5) не возвращает значение и просто приостанавливает выполнение текущего потока

        Task.Delay(5) возвращает объект Task, который можно ожидать (await), что позволяет использовать его в асинхронных методах и управлять потоком выполнения
