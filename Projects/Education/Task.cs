...
namespace SharpEdu
{
    class Tasks
    {
        ...
        static void ForEach_()
        {
            ...
            {
                // source - коллекция, обрабатываемая в цикле
                // body - метод, который будет выполняться на каждом шаге цикла
                // Метод, передаваемый через body, принимает значение | ссылку на каждый элемент массива, но не индекс
                // Возвращаются сведения о состоянии цикла
                // Параллельное выполнение цикла можно остановить, вызвав Break для экземпляра объекта типа ParallelLoopState,
        ...
        static void ParallelForEach_()
        {
            ...
            ParallelLoopResult result = Parallel.ForEach<int>(new List<int>() { 1, 3, 5, 8 }, F1);
        }
