    ...
    */
    ...
    /*
        ...
        Task.WaitAll():
            ...
            - Синхронный метод - блокирует вызывающий поток, пока все переданные задачи не завершатся
            ...
            - Используется, когда необходимо синхронно дождаться завершения задач, например, в консольных приложениях или в методах,
              где асинхронное выполнение не требуется

        Task.WhenAll():

            - Асинхронный метод: - возвращает задачу, которая завершится, когда все переданные задачи завершатся
              Это позволяет продолжать выполнение кода в вызывающем потоке, не блокируя его
            ...
            - Предпочтительно использовать в асинхронных методах и контекстах, где важно не блокировать поток,
