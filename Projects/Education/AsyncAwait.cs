...
public class AsyncAwait
{
    ...
    // В асинхронных методах использовать cancellationToken
    ...
    public static void F18()
    {
        /*
            ...
            Позволяет управлять, как продолжение выполнения асинхронной задачи будет выполняться после завершения ожидания

            При использовании await выполнение метода приостанавливается, пока не завершится асинхронная операция
            После завершения операции управление возвращается к следующему коду

            В большинстве приложений, особенно в UI, после завершения асинхронной операции
            управление возвращается в тот-же контекст синхронизации (например, поток UI),
            чтобы безопасно обновлять элементы интерфейса

            1.ConfigureAwait(true):

               - Поведение по умолчанию

               - После завершения асинхронной операции продолжение будет выполнено в том же контексте,
                 в котором была вызвана операция
                 При вызове метода из UI-потока продолжение так-же выполнится в UI-потоке

               - Это может быть важно для обновления интерфейса пользователя, но может привести к блокировке потока,
                 если вызывающий код ожидает завершения других операций

               - Используйте ConfigureAwait(true) (или просто не указывайте его),
                 когда нужно гарантировать, что продолжение будет выполнено в том-же контексте,
                 например, если вы работаете с элементами интерфейса пользователя

            2.ConfigureAwait(false):

               - Указывает, что продолжение не должно выполняться в исходном контексте

               - Позволяет продолжению выполняться в любом доступном потоке,
                 что может улучшить производительность и избежать блокировок

               - Полезно в ситуациях, когда не нужно возвращаться к контексту, например,
                 при выполнении фоновых операций, которые не требуют доступа к UI

               - Используйте ConfigureAwait(false),
                 когда знаете, что не будете взаимодействовать с UI или контекстом синхронизации после завершения асинхронной операции
                 Это важно в библиотечном коде или при написании серверных приложений, где контекст не имеет значения
        ...
        */
        ...
        {
            // Ожидание асинхронной операции без ConfigureAwait
            // Здесь будет выполнено в том же контексте
            await SomeAsyncOperation();

            // Ожидание асинхронной операции с ConfigureAwait(false)
            // Здесь может быть выполнено в другом потоке
