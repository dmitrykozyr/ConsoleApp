//using Education.Patterns.Behavioral.Command;
...
//    #region Настройка IIS

//        // Подключение к IISAdmin
//        Enter-PSSession -ComputerName localhost -ConfigurationName IISAdmins

//        // Просмотр списка приложений IIS
//        Get-WebApplication -Site "Default Web Site"

//        // Запуск WebPool, если он не запущен
//        Start-WebAppPool -Name ".NET v8.0"

//        // Проверка статуса WebPool
...
//    #region Пайплайн в GitLab CI 

//        /*
//            build → test → quality → package → deploy
//			    - Сборка и юнит-тесты на каждый push
//			    - интеграционные тесты — на merge request

//            Гейты качества:
//			    - если упал хотя бы один тест — всё блокируется
//			    - порог покрытия кода тестами
//			    - робот собирает билд один раз
//                  Он идет сначала на dev, потом на preprod, потом на prod
//                  Нельзя собирать программу заново специально для продакшена
//                  При повторной сборке в сборку может попасть другая версия библиотеки
//        */
