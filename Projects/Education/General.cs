...
namespace SharpEdu
{
    class Program
    {
        class General_
        {
            ...
            Когда программа запускается, то в конце блока памяти,
            зарезервированного для стека, устанавливается указатель
            
            При вызове каждого метода, в стеке будет выделяться область памяти,
            где будут храниться значения переменных

            Стек имеет размер 2 Мб

            Если параметр или переменная метода представляет значимый тип,
