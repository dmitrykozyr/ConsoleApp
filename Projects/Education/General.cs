...
class Program
{
    ...
    {
        /*         
            При использовании ключевого слова yield return:

            - метод возвращает элементы по одному при каждом вызове итератора
            - когда метод использует yield return, он становится итератором,
              который возвращает элементы по одному, сохраняя состояние между вызовами,
              что позволяет экономить память и время, так как элементы генерируются по мере необходимости, а не создаются все сразу
        */
