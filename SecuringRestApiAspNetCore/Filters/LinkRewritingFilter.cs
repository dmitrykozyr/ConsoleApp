...
using SecuringRestApiAspNetCore.Extensions;
...
namespace SecuringRestApiAspNetCore.Filters
{
    public class LinkRewritingFilter : IAsyncResultFilter
    {
        ...
        private static void RewriteAllLinks(object model, LinkRewriter rewriter)
        {
            ...
            var linkProperties = allProperties.Where(z => z.CanWrite && z.PropertyType == typeof(Link));
            ...
            foreach (var linkProperty in linkProperties)
            {
                ...
                {
                    allProperties.SingleOrDefault(z => z.Name == nameof(Resource.Href))
                                 ?.SetValue(model, rewritten.Href);

                    allProperties.SingleOrDefault(z => z.Name == nameof(Resource.Method))
                                 ?.SetValue(model, rewritten.Method);

                    allProperties.SingleOrDefault(z => z.Name == nameof(Resource.Relations))
                                 ?.SetValue(model, rewritten.Relations);
                }
            }

            var arrayProperties = allProperties.Where(z => z.PropertyType.IsArray);
