...
docker push                             запушить репозиторий в hub.docker.com, там есть только один приватный бесплатный репозиторий, пушить необязательно
...
docker run -p 3000:3000 10a68f3b51b7    запускаем контейнер на основе образа и указываем два порта:
                                        первый - локальный, второй - какой порт из контейнера хотим замапить на локальный
