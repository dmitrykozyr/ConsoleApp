...
namespace SharpEdu
{
    ...
    {
        #region==== Общее ===========================================================
        // TPL Библиотека распараллеливания задач усовершенствует многопоточное программирование
        // Упрощает создание и применение многих потоков
        // Позволяет автоматически использовать несколько процессоров
        // Позволяет автоматически распределять нагрузку приложений между доступными процессорами в динамическом режиме, используя пул потоков CLR
        // Класс Task отличается от класса Thread тем, что является абстракцией, представляющей асинхронную операцию, а в классе Thread инкапсулируется поток исполнения
        // Исполнением задач управляет планировщик задач, который работает с пулом потоков, поэтому несколько задач могут разделять один поток
        // В отличие от класса Thread, в классе Task нет свойства Name для хранения имени задачи, но есть свойство Id
        // Каждая задача получает идентификатор при создании, значения уникальны, но не упорядочены, поэтому один идентификатор задачи может появиться перед другим и может и не иметь меньшее значение
        ...
        // В классе Task реализуется интерфейс IDisposable, в котором определяется метод Dispose, освобождающий ресурсы
        ...
        // Можно создать продолжение задачи - это задача, которая автоматически начинается после завершения другой

        // public Task ContinueWith(Action<Task> действие_продолжения)

        // Чтобы возвратить результат из задачи, достаточно создать эту задачу, используя обобщенную форму Task<TResult> класса Task

        // public Task(Func<TResult> функция)
        // public Task(Func<Object, TResult> функция, Object состояние)

        // Функция обозначает выполняемый делегат типа Func - используется, когда задача возвращает результат
        // В 1м конструкторе создается задача без аргументов, во 2м - задача, принимающая аргумент типа Object
        // Значение, возвращаемое задачей, получается из свойства Result в классе Task
        ...
        #region==== Task ============================================================
        // Task - использует работы пул потоков
        // Если false (по умолчанию), то после завершения main ожидается завершение
        // недовыполненных task, иначе они прерываются
        ...
        // этому мы можем вызвать после выполнения одной задачи несколько других, определить условия их вызова, передать из предыдущей задачи в следующую некоторые данные
        ...
        public static void F8()
        {
            ...
            // задача продолжения
            Task task2 = task1.ContinueWith(F8_1);
            task1.Start();
        ...
        // Например, Task<int> - в данном случае задача будет возвращать объект int
        // В качестве задачи должен выполняться метод, возвращающий данный тип объекта
        // Например, первом случае в качестве задачи выполняется функция Factorial, которая принимает числовой параметр и также на выходе возвращает число
        ...
        // Сначала получается признак отмены ПО из источника ПО, затем он передается задаче,  затем она контролирует его на предмет получения запроса на отмену
        // Если получен запрос на отмену, задача должна завершиться
        // В одних случаях этого достаточно для прекращения задачи, а в других - из задачи должен быть вызван метод ThrowIfCancellationRequested() для ПО
        ...
        // Если ПО, для которого вызывается этот метод, получил запрос на отмену, в нем генерируется исключение OperationCanceledException
        // В отменяющем коде можно организовать отслеживание упомянутого исключения
        // Для этого сначала перехватывается исключение AggregateException, затем его внутреннее исключение анализируется с помощью свойства InnerException или InnerExceptions - коллекции исключений
        // ПО получается из источника ПО - объекта класса CancellationTokenSource
        // ПО, связанный с данным источником, доступнен через используемое только для чтения свойство Token
        // Для отмены в задаче должна быть получена копия ПО и организован его контроль с целью отслеживать саму отмену через опрос        
        ...
        // Для создания задачи, из которой вызывается метод ThrowIfCancellationRequested(), когда она отменяется, требуется передать ПО задаче и конструктору класса Task через метод StartNew()
        ...
        // Внутри цикла проверяется свойство IsCancellationRequested - если true, то выводится сообщение об отмене и вызывается метод ThrowIfCancellationRequested() для отмены задачи
        ...
        // Когда сообщение AggregateException генерируется в ответ на отмену задачи - это еще не свидетельствует об ошибке, а означает отмену задачи
        ...
        // В параллельной запущеной задаче через 400 миллисекунд происходит вызов cancelTokenSource.Cancel(), в результате программа выбрасывает исключение OperationCanceledException,
        ...
        public static void F14()
        {
            ...
            try
            {
                Parallel.ForEach<int>(new List<int>() { 1, 2, 3, 4, 5, 6, 7, 8 }, new ParallelOptions { CancellationToken = token }, F14_1);
                // или так
        ...
        // Поддерживает два статических метода - Parallel.For() и Parallel.ForEach(), для каждого из которых определены перегруженные версии
        // Эти методы позволяют создавать тело операторов кода, которое может выполнятся в параллельном режиме
        // Эти операторы представляют собой логику того же рода, которая была бы написана с использованием ключевых слов C# for и foreach
        // Их преимущество в том, что класс Parallel сам берет потоки из пула потоков и управляет конкуренцией
        // Оба эти метода требуют указания совместимого с IEnumerable или IEnumerable<T> контейнера, хранящего данные, которые нужно обработать в параллельном режиме
        // Контейнер может быть простым массивом, необобщенной коллекцией (вроде ArrayList), обобщенной коллекцией (наподобие List<T>) или результатом запроса LINQ
        ...
        // Метод Invoke(), определенный в классе Parallel, позволяет выполнять один или несколько методов, указываемых в виде его аргументов.Он также масштабирует исполнение кода,
        // используя доступные процессоры, если имеется такая возможность.
        ...
        // Все функции параллельного выполнения метод Invoke() берет на себя.И хотя это не гарантирует, что методы будут действительно выполняться параллельно, тем не менее
        ...
        // Поэтому применять метод Invoke() показанным здесь способом нельзя, если требуется, чтобы исполнение вызывающего потока продолжалось
        ...
        // fromInclusive обозначает начальное значение того, что соответствует переменной управления циклом; оно называется также итерационным, или индексным, значением;
        ...
        // Этот метод должен быть совместим с делегатом Action<int>
        ...
        public static ParallelLoopResult ForEach<TSource>(IEnumerable<TSource> source, Action<TSource, ParallelLoopState> body) { return new ParallelLoopResult(); }
        #endregion

        #region==== Вложенные задачи ================================================
        // Одна задача может запускать другую - вложенную задачу, эти задачи выполняются независимо друг от друга и вложенная задача может завершить выполнение после завершения метода Main
        ...
        // Можно столкнуться с тем, что все задачи из массива могут завершиться после того, как отработает метод Main, в котором запускаются эти задачи:
        ...
        // Если необходимо выполнять некоторый код после того как все задачи массива завершатся, то применяется метод Task.WaitAll(tasks)
        ...
        // Он имеет следующее определение: For(int, int, Action<int>), где первый параметр задает начальный индекс элемента в цикле, а второй параметр - конечный индекс
        ...
        // Таким образом, метод будет вести итерацию с 1 до 9 включительно.Третий параметр представляет метод, подсчитывающий факториал числа
        ...
        // Он имеет следующее определение: ParallelLoopResult ForEach<TSource>(IEnumerable<TSource> source,Action<TSource> body), где первый параметр представляет перебираемую коллекцию,
        // а второй параметр - делегат, выполняющийся один раз за итерацию для каждого перебираемого элемента коллекции
        ...
        //И если счетчик в цикле достигнет значения 5, вызывается метод Break
