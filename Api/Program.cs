...
namespace Api
{
    /*
        ...
        На физическом уровне оперируем таблицами, а на концептуальном - объектами
    
        EF предполагает три возможных способа взаимодействия с БД:
        - DB first:     EF создает классы, отражающие модель БД
        - Model first:  сначала создается модель БД, по которой EF создает реальную БД
        - Code first:   сначала создается класс модели данных, которые будут храниться в БД, затем EF генерирует БД и таблицы

        В EF есть три способа загрузки данных:
        ...
        - lazy loading - ленивая загрузка
          Данные подгружаются при первом обращении к навигационному св-ву
